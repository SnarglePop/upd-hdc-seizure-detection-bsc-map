...
if __name__ == "__main__":
                    ...
                    for exc in files_patient:
                        ...
                        for file in files_patient:
                            if exc == file:
                                ...
                                if arch == 2:
                                    tot_arr = torchhd.BSBCTensor.identity(wins, int(n/5), block_size=5)
                                    win_arr = torchhd.BSBCTensor.identity(num_chs, int(n/5), block_size=5)
                                else:
                                ...
                                for window in feat_arr:
                                    ...
                                    feat_len = len(window)
                                    if arch == 2:
                                        sp_arr = torchhd.BSBCTensor.identity(6, int(n/5), block_size=5)
                                        lbp_arr = torchhd.BSBCTensor.identity(feat_len-1, int(n/5), block_size=5)
                                    else:
                                    ...
                                    if chan == 16:
                                        ...
                                        if wincount != 0:
                                            ...
                                            if arch == 2:
                                                prev_win_arr = torchhd.BSBCTensor.identity(1, int(n/5), block_size=5)
                                            else:
    ...
    # TESTING
    patientn = 1
    testing_patients = ["chb"+str(x).zfill(2) for x in range(patientn, 1+1)]
    architecture = [0]
    testing_features = [1]
    log_time = datetime.today().strftime('%Y-%m-%d %H:%M:%S')
                    ...
                    for exc in files_patient:
                        ...
                        if arch == 0:
                            try:
                                ...
                                last_working = counter
                            except:
                                am = [torchhd.BSCTensor(np.loadtxt(f"assocmem/{archi}/feature{feat}/{patient}/seizures/{files_seizures[counter]}.txt")),torchhd.BSCTensor(np.loadtxt(f"assocmem/{archi}/feature{feat}/{patient}/non-seizures/{files_nonseizures[last_working]}.txt"))]                                 
                        elif arch == 1:
                            try:
                                ...
                                am = [torchhd.MAPTensor(np.loadtxt(f"assocmem/{archi}/feature{feat}/{patient}/seizures/{files_seizures[counter]}.txt")),torchhd.MAPTensor(np.loadtxt(f"assocmem/{archi}/feature{feat}/{patient}/non-seizures/{files_nonseizures[last_working]}.txt"))]
                        elif arch == 3:
                            am = [torchhd.HRRTensor(np.loadtxt(f"assocmem/{archi}/feature{feat}/{patient}/seizures/{files_seizures[counter]}.txt")),torchhd.HRRTensor(np.loadtxt(f"assocmem/{archi}/feature{feat}/{patient}/non-seizures/{files_nonseizures[counter]}.txt"))]
                        feat_arr = np.loadtxt(path)
                        if feat == 1:
                            shape = int(len(feat_arr[0])/17)
                        feat_arr = feat_arr.reshape(-1,shape)
