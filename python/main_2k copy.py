...
if __name__ == "__main__":
    ...
    # TRAINING
    # """
    training_patients = ["chb"+str(x).zfill(2) for x in range(1, 6+1)]
    ...
    patientn = 1
    testing_patients = ["chb"+str(x).zfill(2) for x in range(patientn, 6+1)]
    architecture = [0]
    testing_features = [1]
    log_time = datetime.today().strftime('%Y-%m-%d %H:%M:%S')
                ...
                for result in ["seizures", "non-seizures"]:
                    ...
                    for exc in files_patient:
                        ...
                        for i in range(wins):
                            window = feat_arr[i*17:(i+1)*17].transpose()
                            window = window[1:]
                            np.savetxt(f"lbp_window1/lbp_win{i}.txt",window[:128],fmt='%d')
                            win_arr = np.zeros((len(window),n), dtype=int)
                            wincount = 0
                            # win_arr[0] = lastsamp
                            ...
                            for j in win_arr:
                                ...
                                #     f.write(f"{lasttotal}...\n")
                            together = np.sum(win_arr, axis=0)
                            twofiftysix = np.count_nonzero(together == 256)
                            print(twofiftysix)
                            if twofiftysix:
                                with open("256.txt", 'a') as f:
                                    f.write(f"win {i+1} of {wins} for file {counter} of {patient} - {twofiftysix}")
                            res = hdc.bundle(win_arr)
                            # print(np.sum(win_arr, axis=0))
