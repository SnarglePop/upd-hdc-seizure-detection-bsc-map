...
def get_memory(mem_type: int) -> np.ndarray:
    ...
    if mem_type == 0: # Channels
        ...
        memory_chs = np.loadtxt("memory/feature1/BSC_0.txt", int)
        return memory_chs
        ...
        memory_LBP = np.loadtxt("memory/feature1/BSC_1.txt", int)
        return memory_LBP
        ...
        # memory_spi = np.loadtxt("memory/feature2/BSC_1.txt", int)
        # memory_spv = np.loadtxt("memory/feature2/BSC_2.txt", int)
...
def train_leave_one_out(n, window_size, window_step, fs, feature_set, d, memory, label, list_patients, levels = None):
    ...
    for patient in pbar_all:
        ...
        for file_exc in pbar_patient:
            ...
            dump = f"main_dumps/old_{file_exc[:-4]}.txt"
            np.savetxt(dump, label_counters)

            label_hv = hdc.binarize_cont(label_counters)

            if label == "seizures":
                directory = f"training-data/{patient}/s_set{feature_set}_{file_exc[:-4]}.npy"
                # directory = f"assocmem/BSC/feature{feature_set}/{patient}/seizures/new_{file_exc[:-4]}.txt"
                # directory2 = f"assocmem/BSC/feature{feature_set}/{patient}/seizures/new_{file_exc[:-6]}.txt"
                # np.savetxt(directory, label_hv)
                # np.savetxt(directory2, label_hv)
            elif label == "non-seizures":
                directory = f"training-data/{patient}/ns_set{feature_set}_{file_exc[:-4]}.npy"
                # directory = f"assocmem/BSC/feature{feature_set}/{patient}/non-seizures/new_{file_exc[:-4]}.txt"
                # directory2 = f"assocmem/BSC/feature{feature_set}/{patient}/non-seizures/new_{file_exc[:-4]}_0.txt"
                # np.savetxt(directory, label_hv)
                # np.savetxt(directory2, label_hv)
...
def test(test_filepath, n, window_size, window_step, fs, feature_set, d, memory, levels, seizure_filepath, non_seizure_filepath):
    ...
    # seizure_hv = np.loadtxt(seizure_filepath, int)
    with open(non_seizure_filepath, 'rb') as f:
        non_seizure_hv = np.load(f)
    # non_seizure_hv = np.loadtxt(non_seizure_filepath, int)
...
if __name__ == "__main__":
    ...
    training_patients = ["chb"+str(x).zfill(2) for x in range(21, 24+1)]
        ...
        for patient in testing_patients:
            ...
            for patient_file in sorted(os.listdir("chbmit-eeg-processed/seizures/"+patient)):
                num_seiz, num_non_seiz = test("chbmit-eeg-processed/seizures/"+patient+"/"+patient_file, 
                    ...
                    memory=memory,
                    levels=levels,
                    seizure_filepath=str("training-data/" + patient + "/s_" + "set" + str(set) + "_" + patient_file[:-4] + ".npy"),
                    non_seizure_filepath=str("training-data/" + patient + "/ns_" + "set" + str(set) + "_" + patient_file[:-6] + ".npy"))
                # "training-data/" + patient + "/ns_" + "set" + str(set) + "_" + patient_file[:-6] + ".npy"
                # f"assocmem/BSC/feature{set}/{patient}/seizures/new_{patient_file[:-4]}.txt"
            ...
            for patient_file in sorted(os.listdir("chbmit-eeg-processed/non-seizures/"+patient)):
                num_seiz, num_non_seiz = test("chbmit-eeg-processed/non-seizures/"+patient+"/"+patient_file, 
                    ...
                    non_seizure_filepath=str("training-data/" + patient + "/ns_" + "set" + str(set) + "_" + patient_file[:-4] + ".npy"))
