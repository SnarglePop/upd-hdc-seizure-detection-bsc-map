...
if __name__ == "__main__":
    ...
    # TRAINING
    # """
    training_patients = ["chb"+str(x).zfill(2) for x in range(4, 4+1)]
            ...
            for patient in training_patients:
                for result in ["seizures", "non-seizures"]:
                    ...
                    for exc in files_patient:
                        ...
                        files_arr = np.zeros((n,files_count))
                        for file in files_patient2:
                            if exc == file: 
                                ...
                                print(wins)
                                tot_arr = np.zeros((n,wins))
                                for i in range(wins):
                                    window = feat_arr[i*17:(i+1)*17]
                                    for j in range(17):
                                        with open("main_dumps/chb04_preextract.txt", 'a') as log:
                                                log.write(dedent(
                                                    f"""
                                                    {j} - {window[j][1:]}
                                                    """
                                ...
                                for i in range(wins):
                                    ...
                                    window = window[1:]
                                    win_arr = np.zeros((n,len(window)))
                                    wincount = 0
                                    for samp in window:
                                        # print(samp)
                                        samp_arr = np.zeros((n,num_chs))
    ...
    patientn = 1
    testing_patients = ["chb"+str(x).zfill(2) for x in range(patientn, 14+1)]
    architecture = [0]
    testing_features = [1]
    log_time = datetime.today().strftime('%Y-%m-%d %H:%M:%S')
